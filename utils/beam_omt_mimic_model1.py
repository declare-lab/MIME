    ...
    def beam_search(self, src_seq, max_dec_step, emotion_classifier='built_in'):
        ...
        def beam_decode_step(inst_dec_beams, len_dec_seq, src_seq, enc_output, v, inst_idx_to_position_map, n_bm, enc_batch_extend_vocab, extra_zeros, mask_src, encoder_db, mask_transformer_db, DB_ext_vocab_batch):
            ...
            def predict_word(dec_seq, dec_pos, src_seq, enc_output, n_active_inst, n_bm, enc_batch_extend_vocab, extra_zeros, mask_src, encoder_db, mask_transformer_db, atten):
                ...
                if config.USE_CUDA:
                    mask_src_with_emo = torch.cat([torch.zeros((mask_src.shape[0], 1, 1), out=torch.BoolTensor()).cuda(), mask_src], dim=2)
                else:
                    mask_src_with_emo = torch.cat([torch.zeros((mask_src.shape[0], 1, 1), out=torch.BoolTensor()), mask_src], dim=2)
                # mask_src_with_emo = torch.cat([torch.zeros((mask_src.shape[0], 1, 1), out=torch.ByteTensor()).cuda(), mask_src], dim=2)
...
# Code copied from main_mimic_vader.py
                if config.decoder == 'mul':
                    dec_output, attn_dist = self.model.decoder(self.model.embedding(dec_seq), encoder_outputs,
                                                    (mask_src, mask_trg),
                                                    atten)
                elif config.decoder == 'single':
                    dec_output, attn_dist = self.model.decoder(self.model.embedding(dec_seq), v, v, (mask_src, mask_trg))
        ...
        with torch.no_grad():
            ...
            if config.model != 'mimic':
                ...
                if emotion_classifier == "vader":
                    ...
                    emotions_mimic, emotions_non_mimic, mu_p, logvar_p, mu_n, logvar_n = self.vae_sampler(q_h, batch['program_label'], self.model.emoji_embedding)
...
#             logit_prob = self.model.decoder_key(q_h) 
            ...
            if(config.topk>0):
                ...
                if config.USE_CUDA:
                    mask = mask.cuda()
                    k_max_index = k_max_index.cuda()
                logit_prob = mask.scatter_(1,k_max_index.long(),k_max_value)

            attention_parameters = self.model.attention_activation(logit_prob)

            if(config.oracle): 
                attention_parameters = self.model.attention_activation(torch.FloatTensor(src_seq['target_program'])*1000)
                if config.USE_CUDA: attention_parameters = attention_parameters.cuda()
