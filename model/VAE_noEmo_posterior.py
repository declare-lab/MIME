...
class VAESampling(nn.Module):
    def __init__(self, hidden_dim, posterior_hidden_dim, out_dim=32):
        ...
        if config.USE_CUDA:
            self.positive_emotions_t = torch.LongTensor(self.positive_emotions).cuda()
            self.negative_emotions_t = torch.LongTensor(self.negative_emotions).cuda()
        else:
            self.positive_emotions_t = torch.LongTensor(self.positive_emotions)
            self.negative_emotions_t = torch.LongTensor(self.negative_emotions)
        # Prior encoding
    ...
    def posterior(self, x, e, M_out, M_tilde_out):
        if config.USE_CUDA:
            h1_positive = torch.zeros(M_out.shape).cuda()
            h1_negative = torch.zeros(M_out.shape).cuda()
        else:
    ...
    def forward(self, q_h, e, emb_layer):
        ...
        if config.USE_CUDA:
            emotions_mimic = emotions_mimic.cuda()
            emotions_non_mimic = emotions_non_mimic.cuda()
        else:
            emotions_mimic = emotions_mimic
            emotions_non_mimic = emotions_non_mimic
