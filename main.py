...
from model.transformer_mulexpert import Encoder, Decoder, MulDecoder, Generator, MulDecoder, ACT_basic, Transformer_experts
...
class TransformerMimic(nn.Module):
    '''
    ...
    def save_model(self, running_avg_ppl, iter, f1_g, f1_b, ent_g, ent_b, ent_t):
        ...
        model_save_path = os.path.join(self.model_dir,
                'model_{}_{:.4f}_{:.4f}_{:.4f}_{:.4f}_{:.4f}_{:.4f}'.format(iter, running_avg_ppl, f1_g, 
                    f1_b, ent_g, ent_b, ent_t))
    ...
    def train_one_batch(self, batch, iter, train=True):
        ...
        if config.softmax:
            ...
            if config.emo_combine == 'gate':
                ...
                loss = self.criterion(logit.contiguous().view(-1, logit.size(-1)),dec_batch.contiguous().view(-1)) + KLLoss + L1_loss
            else:
                L1_loss = nn.CrossEntropyLoss()(logit_prob, torch.LongTensor(batch['program_label']) if not config.USE_CUDA else torch.LongTensor(batch['program_label']).cuda())
                loss = self.criterion(logit.contiguous().view(-1, logit.size(-1)), dec_batch.contiguous().view(-1)) + KLLoss + L1_loss
        ...
        for _, row in enumerate(np.transpose(decoded_words)):
            ...
            for e in row:
                if e == '<EOS>':
                    ...
                    st += e + ' '
            sent.append(st)
        return sent, batch['context_emotion_scores'][0]['compound'], int(emo_pred[0].data.cpu())
...
if __name__ == '__main__':
    if(config.test):
        ...
        model = TransformerMimic(vocab, decoder_number=program_number, model_file_path=config.saved_model_path, is_eval=True)
        if (config.USE_CUDA):
            model.cuda()
        model = model.eval()
    ...
    model = TransformerMimic(vocab, decoder_number=program_number)
